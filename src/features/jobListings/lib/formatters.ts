...
export function formatWageInterval(interval: (typeof wageIntervals)[number]) {
  switch (interval) {
    case "hourly":
      return "Hour"
    case "yearly":
      return "Year"
    default:
      throw new Error(`Invalid wage interval: ${interval as string}`)
  }
}
...
export function formatWage(wage: number, wageInterval: (typeof wageIntervals)[number]) {
  ...
  switch (wageInterval) {
    ...
    default:
      throw new Error(`Unknown wage interval: ${wageInterval as string}`)
  }
}
